...
/**
 * OAuthCallback — handles the GitHub redirect back to the app.
 *
 * GitHub redirects to /auth/github/callback?code=...&state=...
 * This page reads the params, POSTs them to the backend, and
 * on success stores the tokens and navigates to the dashboard.
 */
export const OAuthCallback = () => {
    ...
    useEffect(() => {
        ...
        const finish = async () => {
            try {
                const res = await fetch('/api/v1/auth/github/callback', {
                    ...
                    body: JSON.stringify({ code, state: 'github' }),
                });
        ...
        finish();
    }, [code, error, navigate, loginWithOAuthTokens]);

    // Show a simple loading state while the exchange is in flight
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-6">
            ...
            {error ? (
                ...
                <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-slate-500 text-sm font-medium">Completing sign-in with GitHub…</p>
                </div>
            )}
        </div>
    );
